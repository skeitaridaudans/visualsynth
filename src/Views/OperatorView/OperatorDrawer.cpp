...
void OperatorDrawer::update(Operator &operator_) {
    ...
    // When the operator is initially pressed_
    ...
    // While holding down when the operator hasn't been moved
        ...
        // When operator is pressed_ (touch is released before dragging starts)
...
void OperatorDrawer::draw(QPainter *painter, const Operator &operator_) {
    ...
    for (const auto opId: operator_.modulatedBy) {
        ...
        const auto modulatorPos = closestPointInRect(modulatedOpPos, QRectF(modulatorOpPos, QSizeF(modulatorBoxSize, modulatorBoxSize)));
        const auto modulatedPos = closestPointInRect(modulatorOpPos, QRectF(modulatedOpPos, QSizeF(modulatedBoxSize, modulatedBoxSize)));
